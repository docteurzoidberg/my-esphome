...
namespace esphome {
namespace zillo_web_server {

static const char *const TAG = "zillo_web_server";
...
void WebServer::setup() {
  ...
  this->events_.onConnect([this](AsyncEventSourceClient *client) {
    ...
    client->send(json::build_json([this](JsonObject root) {
                   ...
                   root["lang"] = "en";
                   root["w"] = this->zillo_->get_width();
                   root["h"] = this->zillo_->get_height();
                 }).c_str(),
                 "ping", millis(), 30000);
  ...
  this->base_->get_server()->onRequestBody([&](AsyncWebServerRequest *request, uint8_t *data, size_t len, size_t index, size_t total){
    if (request->url() == "/setbuffer") {
      if (!this->handle_buffer_request(request, data, len, index, total))
...
bool WebServer::handle_buffer_request(AsyncWebServerRequest *request, uint8_t *data, size_t len, size_t index, size_t total ) {
  //read body data
  ESP_LOGD(TAG, "Body length: %d, index: %d, total: %d", len, index, total);
  //for(int i=0; i<len;i++) {
   // ESP_LOGD(TAG, "byte: %02x", data[i]);
  //}
  if(!index){
    this->zillo_->drawBufferStart();
  }
  this->zillo_->drawBufferAt(index, data, len);
  if(index + len == total){
...
void WebServer::handle_setpixel_request(AsyncWebServerRequest *request, const UrlMatch &match) {
  ...
  if (request->hasParam("x")) {
    x = atoi(request->getParam("x")->value().c_str());
  }
  if (request->hasParam("y")) {
    y = atoi(request->getParam("y")->value().c_str());
  }
  if (request->hasParam("r")) {
    r = atoi(request->getParam("r")->value().c_str());
  }
  if (request->hasParam("g")) {
    g = atoi(request->getParam("g")->value().c_str());
  }
...
bool WebServer::canHandle(AsyncWebServerRequest *request) {
  ...
  if ((request->method() == HTTP_POST || request->method() == HTTP_GET) && match.domain == "setpixel")
    return true;
  ...
  if ((request->method() == HTTP_POST || request->method() == HTTP_GET) && match.domain == "setbuffer")
    return true;
...
void WebServer::handleRequest(AsyncWebServerRequest *request) {
  ...
  //if (match.domain == "setbuffer") {
  //  this->handle_buffer_request(request, match);
  //  return;
  //}

  if (match.domain == "setpixel") {
    this->handle_setpixel_request(request, match);
    return;
  }
