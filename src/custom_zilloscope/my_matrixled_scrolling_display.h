...
#define MATRIX_TILE_H       4  // number of matrices arranged horizontally (negative for reverse order)
#define MATRIX_TILE_V       1  // number of matrices arranged vertically (negative for reverse order)
...
cLEDMatrix<MATRIX_TILE_WIDTH, MATRIX_TILE_HEIGHT, HORIZONTAL_ZIGZAG_MATRIX , MATRIX_TILE_H, MATRIX_TILE_V, HORIZONTAL_BLOCKS> matrix;
...
public:
  ...
  // x,y coordinate of the upper left corner
  {
    ...
    for (int vp_x = 0; vp_x < columns; vp_x++) {
      for (int vp_y = 0; vp_y < rows; vp_y++) {
        int pos = matrix.mXY(vp_x,vp_y);
        Color c = this->buffer_color_[vp_pos];
        (*(lights))[pos] = light::ESPColor(c.r, c.g, c.b, c.w);
        vp_pos++;
      }
    }
    return;
  ...
  struct coord to_addressable_lights3(
      light::AddressableLight *lights, int rows, int columns, int x, int y,
      DisplayOrient rotation = DisplayOrient::DISPLAY_ROTATION_0_DEGREES)
  // x,y coordinate of the upper left corner
  {
